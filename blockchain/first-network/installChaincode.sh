...
CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/crowdFunding/javascript
...
REGEX='Package ID: (.*), Label: crowdFundingv1'
if [[ $(${PEER0_ORG1} lifecycle chaincode queryinstalled) =~ $REGEX ]]; then
  PACKAGE_ID_ORG1=${BASH_REMATCH[1]}
else
  echo Could not find package ID for crowdFundingv1 chaincode on peer0.org1.example.com
  exit 1
fi
...
${PEER0_ORG2} lifecycle chaincode install crowdFunding.tar.gz

echo "Installing smart contract on peer1.org2.example.com"
${PEER1_ORG2} lifecycle chaincode install crowdFunding.tar.gz
...
if [[ $(${PEER0_ORG2} lifecycle chaincode queryinstalled) =~ $REGEX ]]; then
  ...
  echo Could not find package ID for crowdFundingv1 chaincode on peer0.org2.example.com
  exit 1
fi
