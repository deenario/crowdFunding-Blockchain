...
logs() {
    LOG_DIRECTORY=$WORKSPACE/tokenWallet/$1
...
cd $WORKSPACE/$BASE_DIR/tokenWallet || exit
...
for LANGUAGE in ${LANGUAGES}; do
    ...
    copy_logs $? tokenWallet-${LANGUAGE}
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        if [ ${LANGUAGE} = "javascript" -o ${LANGUAGE} = "typescript" ]; then
            ...
            copy_logs $? tokenWallet-${LANGUAGE}-enrollAdmin
            ${COMMAND} ${PREFIX}registerUser${SUFFIX}
            copy_logs $? tokenWallet-${LANGUAGE}-registerUser
            ${COMMAND} ${PREFIX}query${SUFFIX}
            copy_logs $? tokenWallet-${LANGUAGE}-query
            ${COMMAND} ${PREFIX}invoke${SUFFIX}
            copy_logs $? tokenWallet-${LANGUAGE}-invoke
